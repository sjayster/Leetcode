...
1. Start the arrays with [1]
2. Iterate over len(nums)-1 and multiple r1[i]*n[i] and r2[i]* n[-1-i]
3. Iterate over range len(nums) and multiply r1[i]* r2[-1-i]
"""
...
class Solution(object):
    ...
    def productExceptSelf(self, nums):
        ...
        for i in range(length - 1):
            r1.append(r1[i] * nums[i])
            r2.append(r2[i] * nums[-1 - i])

        for i in range(length):
            r.append(r1[i] * r2[-1 - i])
